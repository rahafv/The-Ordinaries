...
class ShowUserTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('follow_toggle', kwargs={'user_id': self.followee.id, 'club_id': self.club.id})

    def test_follow_toggle_url(self):
        self.assertEqual(self.url,f'/follow_toggle/{self.followee.id}/{self.club.id}')
    ...
    def test_get_follow_toggle_for_followee(self):
        ...
        response_url = reverse('profile', kwargs={'user_id': self.followee.id, 'club_id': self.club.id})
    ...
    def test_get_follow_toggle_with_invalid_user_id(self):
        self.client.login(username=self.user.username, password='Password123')
        url = reverse('follow_toggle', kwargs={'user_id': self.user.id+9999, 'club_id':self.club.id})
        response = self.client.get(url, follow=True)
        self.assertEqual(response.status_code, 404) 

    def test_get_follow_toggle_with_invalid_club_id(self):
        self.client.login(username=self.user.username, password='Password123')
        url = reverse('follow_toggle', kwargs={'user_id': self.user.id, 'club_id':self.club.id+9999})
        response = self.client.get(url, follow=True)
        self.assertEqual(response.status_code, 404) 
